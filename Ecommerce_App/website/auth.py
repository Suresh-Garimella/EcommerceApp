...
def login():
    ...
    if request.method == 'POST':
        ...
        if account == 'admin':
            ...
            # print("Check if they are same Admin \n")
            # print("Password is :",generate_password_hash("abcd"))
            # # li=[{item: data[item] for item in data if item != '_id'} for data in user]
            # # print(li['password'])
            # for i in user:
            #     print(i)
            # print(user.__getattribute__)
            # # print(type(user))
            # print(user['email'])
            ...
            # if (check_password_hash(user["password"],"abcd")):
            #     print('Hyyyyyyyyyyyyyyyyyyyyyyyyyyyyy correct Password')
            #     print("Your Passswored is ",password)
            # print("Password is :",generate_password_hash(password,method='sha256'))
    ...
    if request.method == "POST":
        ...
        if len(email)<4:
            ...
            try:
                ...
                # print("Password is :",generate_password_hash(password1,method='sha256'))
                input_data = {
                    "firstName":firstName,
                    "email":email,
