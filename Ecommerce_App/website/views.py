...
def returns():
    ...
    print("return_id inside the returns fucntion is",returnid)
    #  To check if it is present in the return table 
    return_item= db.returns.find_one({'return_id':returnid})
    print(return_item)
    print("+++++++++++++++++++++++++++++")
    # print(return_item["item_id"])
    # if int(return_item["item_id"])==int(itemid):
    if return_item!=None:
        flash(' This Item is Already Returned',category='error')
...
def adminaccept():
    ...
    returnid=temp["returnid"][0]
    print("return id inside the AdminAccept is ",returnid)
    db.returns.update_one({"return_id" : returnid},{"$set":{"return_status":"Accepted"}})
    orderid = returnid.split(',')[0] 
    ...
    db.orders.update_one(
        {"order_id":orderid,"item_details.item_id":int(itemid)},
        {$set:
         {"item_details.$.order_status":"Returned"}
        }
    );
    """ 
    # 
